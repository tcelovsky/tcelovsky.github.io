...
_Note: If you use the `build_` option, you’ll have to persist your new `author` with `#save` while the `create_` option will persist to the database for you._
...
Note how workout\_class class serves as a connector between instructor class and student class in the above example. This is that third model that helps to set up a many-to-many connection between students and instructors.
...
Note the assemblies\_parts join table in the above example that allows the `has_and_belongs_to_many` relationship to work.
